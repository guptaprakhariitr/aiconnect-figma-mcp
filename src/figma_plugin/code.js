...
// Resolve "@ref" / "@ref.id" placeholders in a params object against a refs map.
// Walks nested objects/arrays so parentId, nodeId, nodeIds[], etc. all resolve.
function resolveRefs(value, refs) {
  if (typeof value === "string") {
    if (value.length > 1 && value[0] === "@") {
      let key = value.slice(1);
      if (key.endsWith(".id")) key = key.slice(0, -3);
      if (Object.prototype.hasOwnProperty.call(refs, key)) return refs[key];
      return value; // unknown ref → leave as-is so the error surfaces clearly
    }
    return value;
  }
  ...
  if (value && typeof value === "object") {
    const out = {};
    for (const k of Object.keys(value)) out[k] = resolveRefs(value[k], refs);
    return out;
  }
  return value;
}

// Execute many commands in ONE round-trip. Children can reference nodes created
// earlier in the same batch via "@ref" placeholders (e.g. parentId: "@hero").
// params: { ops: [{ ref?, command, params? }, ...], stopOnError?: boolean }
// Returns a compact result: { ok, count, ids: {ref:id}, errors:[{i,ref,command,error}] }.
async function batchOps(params) {
  const ops = (params && params.ops) || [];
  if (!Array.isArray(ops)) throw new Error("batch_ops: 'ops' must be an array");
  const stopOnError = !!(params && params.stopOnError);
  const refs = {};
  const ids = {};
  const errors = [];
  let last = null;
  for (let i = 0; i < ops.length; i++) {
    ...
    if (!command) { errors.push({ i, error: "missing command" }); if (stopOnError) break; continue; }
    try {
      const resolved = resolveRefs(op.params || {}, refs);
      const result = await handleCommand(command, resolved);
      last = result;
      const nodeId = result && (result.id || result.nodeId);
      if (op.ref && nodeId) { refs[op.ref] = nodeId; ids[op.ref] = nodeId; }
      else if (op.ref) { ids[op.ref] = result; } // non-node result, keep raw
    } catch (e) {
      errors.push({ i, ref: op.ref, command, error: (e && e.message) || String(e) });
      if (stopOnError) break;
    }
  }
  return { ok: errors.length === 0, count: ops.length, ran: ops.length, ids, errors, last };
}

// Command implementations

// Set the font family/style on a text node (e.g. Literata Semi Bold)
async function setFontName(params) {
  const { nodeId, family, style } = params || {};
  if (!nodeId) throw new Error("Missing nodeId parameter");
  if (!family) throw new Error("Missing family parameter");
  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) throw new Error(`Node not found with ID: ${nodeId}`);
  if (node.type !== "TEXT") throw new Error(`Node is not a text node: ${nodeId}`);
  const targetFont = { family: family, style: style || "Regular" };
  // Load the node's existing font(s) first (required before mutating font on already-typed text)
  try {
...
// Decode base64 -> Uint8Array (uses figma.base64Decode when available, else manual)
function _b64ToBytes(b64) {
  if (typeof figma !== "undefined" && figma.base64Decode) return figma.base64Decode(b64);
...
// Set a linear gradient fill (premium backgrounds, overlays, buttons).
async function setGradientFill(params) {
  const { nodeId, stops, direction } = params || {};
  if (!nodeId) throw new Error("Missing nodeId parameter");
  const node = await figma.getNodeByIdAsync(nodeId);
  ...
  var t;
  switch (direction) {
    case "horizontal": t = [[1, 0, 0], [0, 1, 0]]; break;
    case "diagonal": t = [[1, 1, 0], [-1, 1, 1]]; break;
    case "vertical":
    default: t = [[0, 1, 0], [-1, 0, 1]]; break;
  }
  const gstops = (stops && stops.length ? stops : [
    { position: 0, color: { r: 1, g: 1, b: 1, a: 1 } },
    { position: 1, color: { r: 0, g: 0, b: 0, a: 1 } }
  ]).map(function (s) {
    var c = s.color || { r: 0, g: 0, b: 0 };
    return { position: s.position, color: { r: c.r, g: c.g, b: c.b, a: (c.a != null ? c.a : 1) } };
  });
  node.fills = [{ type: "GRADIENT_LINEAR", gradientTransform: t, gradientStops: gstops }];
  return { id: node.id, name: node.name, stops: gstops.length, direction: direction || "vertical" };
}

// Create an ellipse (dots, blobs, circular accents).
async function createEllipse(params) {
  const { x, y, width, height, parentId, name, fillColor } = params || {};
  const e = figma.createEllipse();
  e.x = x || 0; e.y = y || 0; e.resize(width || 100, height || 100);
  if (name) e.name = name;
  if (fillColor) {
    var c = fillColor;
    e.fills = [{ type: "SOLID", color: { r: c.r, g: c.g, b: c.b }, opacity: (c.a != null ? c.a : 1) }];
  }
  if (parentId) { const p = await figma.getNodeByIdAsync(parentId); if (p && p.appendChild) p.appendChild(e); }
  return { id: e.id, name: e.name };
}

// Create a vector node from an SVG string (icons, motifs, patterns, dividers, blobs).
async function createSvg(params) {
  const { svg, parentId, x, y, name } = params || {};
  if (!svg) throw new Error("Missing svg parameter");
  const node = figma.createNodeFromSvg(svg);
  if (name) node.name = name;
  if (x != null) node.x = x;
  if (y != null) node.y = y;
  if (parentId) { const p = await figma.getNodeByIdAsync(parentId); if (p && p.appendChild) p.appendChild(node); }
  return { id: node.id, name: node.name, width: node.width, height: node.height };
}

// Set effects (drop/inner shadows, blurs) on a node for depth/premium feel.
async function setEffect(params) {
  ...
  const list = (effects && effects.length ? effects : [{}]).map(function (e) {
    var type = e.type || "DROP_SHADOW";
    if (type === "LAYER_BLUR" || type === "BACKGROUND_BLUR") {
      return { type: type, radius: (e.radius != null ? e.radius : 12), visible: true };
    }
    return {
      type: type, // DROP_SHADOW | INNER_SHADOW
      color: e.color || { r: 0.102, g: 0.18, b: 0.188, a: (e.a != null ? e.a : 0.12) },
      offset: e.offset || { x: (e.x != null ? e.x : 0), y: (e.y != null ? e.y : 10) },
  ...
  node.effects = list;
  return { id: node.id, name: node.name, count: list.length };
}

// Move/reorder a node into a parent at a specific index (enables inserting
// sections between existing ones, and ordering badges within cards).
async function insertChild(params) {
  ...
  var idx = (typeof index === "number") ? index : parent.children.length;
  if (idx < 0) idx = 0;
...
async function setImageFill(params) {
  const { nodeId, imageBase64, scaleMode } = params || {};
  if (!nodeId) throw new Error("Missing nodeId parameter");
  if (!imageBase64) throw new Error("Missing imageBase64 parameter");
  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) throw new Error(`Node not found with ID: ${nodeId}`);
  if (!("fills" in node)) throw new Error(`Node does not support fills: ${nodeId}`);
  const bytes = _b64ToBytes(imageBase64);
  const image = figma.createImage(bytes);
  const mode = scaleMode || "FILL";
  node.fills = [{ type: "IMAGE", scaleMode: mode, imageHash: image.hash }];
...
async function setFillColor(params) {
  ...
  // Accept both { color: { r,g,b,a } } (sent by the MCP set_fill_color tool)
  // and top-level { r,g,b,a } (ergonomic when used inside batch_ops).
  const src = p.color && typeof p.color === "object" ? p.color : p;
  const { r, g, b, a } = src;
