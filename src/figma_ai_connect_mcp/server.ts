...
// Batch Tool — run many commands in ONE round-trip (huge token saver)
server.tool(
  "batch_ops",
  "Execute MANY Figma commands in a single round-trip. Each op = { ref?, command, params? }. " +
    "`command` is any other tool's underlying command name (e.g. create_frame, create_text, set_font_name, " +
    "set_layout_sizing, set_corner_radius, set_image_fill, set_fill_color, set_gradient_fill, set_effect, " +
    "resize_node, clone_node, set_text_content, insert_child, create_svg, create_ellipse, set_axis_align, " +
    "set_padding, set_item_spacing, move_node, delete_node). `params` is that command's normal params object. " +
    "Cross-reference nodes created earlier in the SAME batch with an \"@ref\" placeholder anywhere a node id is " +
    "expected: give an op a `ref` (e.g. ref:\"hero\"), then later use parentId:\"@hero\" (or nodeId:\"@hero\"). " +
    "Refs resolve in nested objects/arrays too. Ops run sequentially (safe — no parallel crashes). " +
    "Returns a compact { ok, count, ids:{ref:nodeId}, errors:[{i,ref,command,error}] } — NOT verbose node dumps. " +
    "Use this to build whole sections/pages at once instead of dozens of individual calls.",
  {
    ops: z
      .array(
        z.object({
          ref: z.string().optional().describe("Optional local name to reference this node later via @ref"),
          command: z.string().describe("Underlying command name, e.g. create_frame"),
          params: z.record(z.any()).optional().describe("Params for that command; values may contain @ref placeholders"),
        })
      )
      .describe("Ordered list of operations to execute in one round-trip"),
    stopOnError: z.boolean().optional().describe("If true, stop at the first failing op (default false: collect errors and continue)"),
  },
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error running batch_ops: ${error instanceof Error ? error.message : String(error)}`,
          },
        ],
      };
    }
...
// Set Font Name Tool (added) — set a text node's font family/style (e.g. Literata)
server.tool(
  "set_font_name",
  "Set the font family and style of a text node, e.g. family 'Literata' style 'Semi Bold'.",
  {
    nodeId: z.string().describe("The ID of the text node"),
    family: z.string().describe("Font family, e.g. 'Literata' or 'Inter'"),
    style: z.string().optional().describe("Font style, e.g. 'Regular', 'Bold', 'Semi Bold'"),
  },
  async ({ nodeId, family, style }: any) => {
    try {
      await sendCommandToFigma("set_font_name", { nodeId, family, style: style || "Regular" });
      return { content: [{ type: "text", text: `Set font of node ${nodeId} to ${family} ${style || "Regular"}` }] };
    } catch (error) {
      return { content: [{ type: "text", text: `Error setting font: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  }
);

// Set Image Fill Tool (added) — import a real image (local path or URL) as a node's fill
server.tool(
  "set_image_fill",
  "Set an image fill on a node from a local file path or an http(s) URL. Imports the real image into Figma.",
  {
    nodeId: z.string().describe("The ID of the node to fill"),
    imagePath: z.string().optional().describe("Absolute local file path to a PNG/JPG/WEBP image"),
    imageUrl: z.string().optional().describe("http(s) URL to an image"),
    scaleMode: z.enum(["FILL", "FIT", "CROP", "TILE"]).optional().describe("Image scale mode (default FILL)"),
  },
  async ({ nodeId, imagePath, imageUrl, scaleMode }: any) => {
    try {
      ...
      const result: any = await sendCommandToFigma("set_image_fill", { nodeId, imageBase64, scaleMode: scaleMode || "FILL" });
      return { content: [{ type: "text", text: `Set image fill on "${result && result.name}" (${bytes.length} bytes, ${scaleMode || "FILL"})` }] };
    } catch (error) {
...
// Set Gradient Fill Tool (added)
server.tool(
  "set_gradient_fill",
  "Set a linear gradient fill on a node. stops: [{position:0..1, color:{r,g,b,a}}]. direction: 'vertical' | 'horizontal' | 'diagonal'.",
  {
    nodeId: z.string().describe("The ID of the node"),
    stops: z.array(z.object({ position: z.number(), color: z.object({ r: z.number(), g: z.number(), b: z.number(), a: z.number().optional() }) })).describe("Gradient color stops"),
    direction: z.enum(["vertical", "horizontal", "diagonal"]).optional().describe("Gradient direction (default vertical)"),
  },
  async ({ nodeId, stops, direction }: any) => {
    try {
      const r: any = await sendCommandToFigma("set_gradient_fill", { nodeId, stops, direction });
      return { content: [{ type: "text", text: `Set ${r && r.stops}-stop ${r && r.direction} gradient on "${r && r.name}"` }] };
    } catch (error) {
      return { content: [{ type: "text", text: `Error setting gradient: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  }
);

// Create Ellipse Tool (added)
server.tool(
  "create_ellipse",
  "Create an ellipse/circle (dots, blobs, circular accents).",
  {
    x: z.number(), y: z.number(), width: z.number(), height: z.number(),
    parentId: z.string().optional().describe("Parent node to append to"),
    name: z.string().optional(),
    fillColor: z.object({ r: z.number(), g: z.number(), b: z.number(), a: z.number().optional() }).optional(),
  },
  async ({ x, y, width, height, parentId, name, fillColor }: any) => {
    try {
      const r: any = await sendCommandToFigma("create_ellipse", { x, y, width, height, parentId, name, fillColor });
      return { content: [{ type: "text", text: `Created ellipse "${r && r.name}" (${r && r.id})` }] };
    } catch (error) {
...
// Create SVG Tool (added) — vector motifs, icons, patterns, dividers
server.tool(
  "create_svg",
  "Create a vector node from an SVG string (icons, brand motifs like molecule/droplet/wave, decorative patterns, dividers, blobs). Returns the node id; reparent/position via insert_child/move.",
  {
  ...
  async ({ svg, parentId, x, y, name }: any) => {
    try {
      const r: any = await sendCommandToFigma("create_svg", { svg, parentId, x, y, name });
      return { content: [{ type: "text", text: `Created SVG "${r && r.name}" (${r && r.id}) ${r && Math.round(r.width)}x${r && Math.round(r.height)}` }] };
    } catch (error) {
...
// Set Effect Tool (added) — drop/inner shadows & blurs for depth
server.tool(
  "set_effect",
  "Set effects (drop shadows, inner shadows, blurs) on a node for depth/premium feel. Pass effects: [{type, color:{r,g,b,a}, offset:{x,y}, radius, spread}]. type defaults to DROP_SHADOW.",
  {
    nodeId: z.string().describe("The ID of the node"),
    effects: z.array(z.object({
      type: z.string().optional().describe("DROP_SHADOW | INNER_SHADOW | LAYER_BLUR | BACKGROUND_BLUR"),
      color: z.object({ r: z.number(), g: z.number(), b: z.number(), a: z.number().optional() }).optional(),
      offset: z.object({ x: z.number(), y: z.number() }).optional(),
      radius: z.number().optional(),
      spread: z.number().optional(),
    })).optional().describe("Array of effects; omit for a default soft drop shadow"),
  },
  async ({ nodeId, effects }: any) => {
    try {
      const result: any = await sendCommandToFigma("set_effect", { nodeId, effects });
      return { content: [{ type: "text", text: `Set ${result && result.count} effect(s) on "${result && result.name}"` }] };
    } catch (error) {
      return { content: [{ type: "text", text: `Error setting effect: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  }
);

// Insert Child Tool (added) — move/reorder a node into a parent at an index
server.tool(
  "insert_child",
  "Move/reorder a node into a parent at a specific index. Enables inserting new sections between existing ones and reordering children (e.g. badges within a card). index 0 = first.",
  {
    parentId: z.string().describe("The ID of the parent (auto-layout) frame"),
    childId: z.string().describe("The ID of the node to move into the parent"),
    index: z.number().optional().describe("Target index (0 = first). Omit to append at end."),
  },
  async ({ parentId, childId, index }: any) => {
    try {
      const result: any = await sendCommandToFigma("insert_child", { parentId, childId, index });
      return { content: [{ type: "text", text: `Inserted ${childId} into ${parentId} at index ${result && result.index} (childCount ${result && result.childCount})` }] };
    } catch (error) {
